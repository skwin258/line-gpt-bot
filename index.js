...
const allowedUsers = new Set([
  'U48c33cd9a93a3c6ce8e15647b8c17f08', // Skwin 無到期
  'Ufaeaa194b93281c0380cfbfd59d5aee0', // 豪 無到期
  'U73759fc9139edfaf7c804509d0a8c07f', // 搗蛋鬼 無到期
  'U63918f9d8b44770747034598a351595e', // Happy’s 無到期
  'U1cebd63109f62550c10df0ab835a900c', // Huang 無到期
  'U0ea07940728c64ae26385f366b5b9603', // 汪 無到期
  'U35cf199d3a707137efed545d782e11c0', // 陳奕豪 無到期
  'Udbc76d0c8fab9a80a1c6a7ef12ac5e81', // 柯凱迪 無到期
  'Uf7c1ad44ebc11e81cb24a2a38b9f3b39', // Kaiii/汪 無到期
  'U2031c52d282931d135d54e21c5657184', // 江易洤/樂 無到期
  'U52dba780e8908d86ee340d1dc22569f0', // 邁爾斯/邁 無到期
  'Ud19fd8e4d6613c269b2828093f26a313', // ARON/汪 無到期
  'U97561cbf50d393700d99e3fbdd06eb1b', // 11/依 無到期
  'U29db95740cf386c992f44e6139443536', // 小羊/洋 無到期
  'U52c2dd03115dd120cd653c78cd1d02c1', // 吳奇軒/樂 無到期
  '000000000000000000000000000000000', // 超級管理員
  'U1e1993bfd0c82a832bec436a6debc3b1', // 羅冠博/樂 11/19 下午2點
  'U11b93b19620bad5f2763460d2c28ad58', // mark/汪 11/18 下午17點
  'U0f0d348e03cc04ad745100e5c9eb0219', // 范洸瑋/搗 11/18 下午17點
  'U27e4285f91ccbac194b773f794035284', // 劉子瑜 / 依 / 11/24 / 20:00
  'U459ee75e033ca58433935e7abc0da32b', // 費世宏 / 依 / 11/18 / 19:30
  'Uf06f18b73c32d5fa3e8241cd138bb616', // 潘曜成 / 依 / 11.19 / 13:30
  'U252b36feb8941911da1b1458aa75610e', // 謝丞晉/搗 11/18 下午22點
  'U8cf59870e1c7bd9599ae27a6b250e6f8', // 陳昀澤/汪 11/20 下午3:10分
  'Ud2f46a7d2ae73402c1b23ad4a0d4a2e4', // 孫文俊/汪 11/20 下午5:30分
  '000000000000000000000000000000000', // 超級管理員
  '000000000000000000000000000000000', // 超級管理員
]);
...
// 新增：桌名正規化，避免空白/符號差異
function normalizeFullTableName(s) {
...
async function handleEvent(event) {
  ...
  // 新增：開始分析（不帶參數 → 用目前選桌）
  if (userMessage === '開始分析') {
    const ft = userCurrentTable.get(userId);
    if (!ft) return safeReply(event, { type: 'text', text: '尚未選擇牌桌。請先選擇系統/廳/桌。' });
    // 轉成帶參數的同一流程
    return handleEvent({ ...event, message: { ...event.message, text: `開始分析|${ft}` } });
  }

  // 開始分析（私聊）
  if (userMessage.startsWith('開始分析|')) {
    const fullTableName = userMessage.split('|').slice(1).join('|').trim();
    const rec = userRecentInput.get(userId);

    const want = normalizeFullTableName(fullTableName);
    const recTable = normalizeFullTableName(rec?.tableFull || '');
    const curr = normalizeFullTableName(userCurrentTable.get(userId) || '');

    // 需有序列且必須是同一張桌子的序列（容許與當前選桌一致）
    const tableOk = !!rec && (recTable === want || recTable === curr);

    if (!rec || !/^[閒莊和]{6,20}$/.test(rec.seq) || !tableOk) {
      return safeReply(event, {
        type: 'text',
